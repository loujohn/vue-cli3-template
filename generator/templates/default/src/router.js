...
export const generateRoutes = (conf, lazyLoader, parent) => {
  ...
  if (conf) {
    ...
    componentPath = conf.template
      ? conf.template
      : path.replace(/^\//g, "") +
        (conf.children && conf.children.length > 0 ? "/index" : "");
    ...
    result = {
      ...
      meta: conf.meta,
      children:
        conf.children && conf.children.length > 0
          ? conf.children.map(c => generateRoutes(c, lazyLoader, path))
          : [],
